...
public class ReportService extends ServiceBase {
    ...
    /**
     * 指定した従業員がフォローしている従業員の日報を、更新日時の降順で、指定されたページ数の一覧画面に表示する分取得しReportViewのリストで返却する
     * @param followerEmployee 従業員
     * @param page ページ数
     * @return 一覧画面に表示するデータのリスト
     */
    public List<ReportView> getFollowedPerPage(EmployeeView followerEmployee, int page) {
        List<Report> reports = em.createNamedQuery(JpaConst.Q_REP_GET_ALL_FOLLOWED, Report.class)
     ...
     * @return 日報データの件数
     */
    public long countAllFollowed(EmployeeView followerEmployee) {

        long count = (long) em.createNamedQuery(JpaConst.Q_REP_COUNT_ALL_FOLLOWED, Long.class)
                .setParameter(JpaConst.JPQL_PARM_FOLLOWER, EmployeeConverter.toModel(followerEmployee))
                .getSingleResult();
