...
@NamedQueries({
    ...
    @NamedQuery(
            name = JpaConst.Q_REP_GET_ALL_FOLLOWED,
            query = JpaConst.Q_REP_GET_ALL_FOLLOWED_DEF),
    @NamedQuery(
            name = JpaConst.Q_REP_COUNT_ALL_FOLLOWED,
            query = JpaConst.Q_REP_COUNT_ALL_FOLLOWED_DEF)
})

@Getter //全てのクラスフィールドについてgetterを自働生成する(Lombok)
@Setter //全てのクラスフィールドについてsetterを自働生成する(Lombok)
@NoArgsConstructor //引数なしコンストラクタを自働生成する(Lombok)
@AllArgsConstructor //全てのクラスフィールドを引数にもつ引数ありコンストラクタを自働生成する(Lombok)
@Entity
public class Report {
     ...
     * いいね数
     */
    @Column(name = JpaConst.REP_COL_FAVORITE_COUNT, nullable = false)
